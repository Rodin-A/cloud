...
class ZipStreamer {
  ...
  const ATTR_MADE_BY_VERSION = 0x062d; // made by version  (upper byte: UNIX, lower byte v4.5)
  ...
  */
  ...
  private function buildCentralDirectoryHeader($filePath, $timestamp, $gpFlags,
    ...
    return ''
      ...
      . pack16le(strlen($filePath))              //file name length                2 bytes
      . pack16le(strlen($zip64Ext))              //extra field length              2 bytes
      . pack16le(strlen($this->comment))         //file comment length             2 bytes
      ...
      . $filePath                                //file name                       (variable size)
      . $zip64Ext                                //extra field                     (variable size)
      . $this->comment;                          //file comment                    (variable size)
  }
  ...
  private function buildEndOfCentralDirectoryRecord($cdRecLength) {
    ...
    return ''
      ...
      . pack16le(strlen($this->comment))             // .ZIP file comment length        2 bytes
      . $this->comment;                              // .ZIP file comment               (variable size)
  }
